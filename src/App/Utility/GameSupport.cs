...
namespace Launcher.App.Utility
{
    ...
    public static class GameSupport
    {
        ...
        public static short RandomItemSkill(string Nickname, byte gameType)
        {
            ...
            {
                Random random = new Random();
                int index = random.Next(MultiPlayer.itemProb_indi.Count);
                short skill = MultiPlayer.itemProb_indi[index];
            ...
            {
                Random random = new Random();
                int index = random.Next(MultiPlayer.itemProb_team.Count);
                short skill = MultiPlayer.itemProb_team[index];
                skill = GetItemSkill(Nickname, skill);
                return skill;
            }
            return 0;
        ...
        public static short GetItemSkill(string Nickname, short skill)
        {
            ...
            if (MultiPlayer.kartConfig.SkillChange.TryGetValue(ProfileService.ProfileConfigs[ProfileService.SettingConfig.Name].RiderItem.Set_Kart, out var changes) &&
